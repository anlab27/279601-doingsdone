...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ((!isset($task['date'])) && validateDate($task['date'])) {
    ...
    if (count($errors)) {
        ...
        $link = connectToMysql();
        $sql = 'INSERT INTO tasks (created_at, completed_status, name, file_path, deadline, user_id, project_id) VALUES (NOW(), 0, ?, ?, ?, 1, ?)';
        $stmt = db_get_prepare_stmt($link, $sql, [$task['name'], $task['path'], $task['date'], $task['project']]);
